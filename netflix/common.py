...
def bic(X: np.ndarray, mixture: GaussianMixture,
    ...
    """
    n, d = X.shape
    K = len(mixture.mu) # Find number of components

    p = K * d + K + (K - 1) # Free parameters, K*d (Each of the K components has a mean vector of dimension d) + K (Each component has a single variance parameter) + 
                            # (K-1) weights (there are K weights, but K-1 are free due to the constraint that they sum to 1.)
